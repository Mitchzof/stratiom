...
import { loadTrustlines, loadAccount, deleteTrustline } from './helpers/stellarHelper';
...
class TrustManager extends Component {
  ...
  render() {;
    let rows = []
    if (this.props.trustlines) {
      this.props.trustlines.forEach(trustline => {
        rows.push(<Trustline key={ trustline.issuer } trustline={ trustline } deleteAction={ this.deleteAction } />);
      });
    } else {
